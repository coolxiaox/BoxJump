...
std::vector<std::string> split(const std::string &s, const std::string &sep)
{
    ...
    int convertStringToMask(std::string& str, int defVal) {
        ...
        printf("str = %s\n", str.c_str());
        int mask = 0;
		std::vector<std::string> valueArray = split(str, ";");
        for (int i = valueArray.size() - 1; i >= 0; i--) {
			//            char ch = str[i];
			//			printf("ch = %c\n", ch);
			std::string ch = valueArray[i];
			printf("ch = %s\n", ch.c_str());
			//            if (ch == ';') continue;
            if (ch == " ") continue;
            if (ch != "0")
				mask |= (1 << (atoi(ch.c_str()) - 1));
        }
	...
	int convertStringToMask(std::string& str) {
		return convertStringToMask(str, 0xFFFFFFFF);
    }
...
Node * CCBReader::readNodeGraph(Node * pParent)
{
    ...
    if (hasPhysicsBody)
    {
        ...
        /*body->setCollisionBitmask(convertStringToMask(collisionType));
        body->setCategoryBitmask(convertStringToMask(collisionCategories));
        body->setContactTestBitmask(convertStringToMask(collisionMask));
		 */
